...
use simplicityhl_std::artifacts::u64_mock::U64MockProgram;
use simplicityhl_std::artifacts::u64_mock::derived_u64_mock::{U64MockArguments, U64MockWitness};
...
fn get_script(context: &simplex::TestContext) -> (U64MockProgram, Script) {
    ...
    let u64_program = U64MockProgram::new(arguments);

    let u64_script = u64_program.get_script_pubkey(context.get_network());

    (u64_program, u64_script)
}

fn fund_script(context: &simplex::TestContext) -> anyhow::Result<()> {
    let signer = context.get_default_signer();

    let (_, u64_script) = get_script(context);

    let tx_receipt = signer.send(u64_script.clone(), 50)?;
...
fn spend_script(
    ...
    let (u64_program, u64_script) = get_script(context);

    let asserts_utxos = provider.fetch_scripthash_utxos(&u64_script)?;
    ...
    ft.add_program_input(
        ...
        ProgramInput::new(
            Box::new(u64_program.as_ref().clone()),
