...
use simplicityhl_std::artifacts::mock::u64_mock::derived_u64_mock::{U64MockArguments, U64MockWitness};
...
fn spend_script(context: &simplex::TestContext, function_index: u8, if_test_overflow: bool, first_arg: u64, second_arg: u64, result: u64,) -> anyhow::Result<()> {
    ...
    let witness = U64MockWitness {function_index: function_index, if_test_overflow: if_test_overflow, first_arg: first_arg, second_arg: second_arg, result: result };
...
fn u64_test_checked_add_64_not_overflow(context: simplex::TestContext) -> anyhow::Result<()> {
    let function_index = FunctionToTest::CheckedAdd64;
...
fn u64_test_safe_add_64_not_overflow(context: simplex::TestContext) -> anyhow::Result<()> {
    let function_index = FunctionToTest::SafeAdd64;
    let if_test_overflow = false;
...
fn u64_test_safe_subtract_64_not_overflow(context: simplex::TestContext) -> anyhow::Result<()> {
    let function_index = FunctionToTest::SafeSubtract64;
    let if_test_overflow = false;
...
fn u64_test_safe_multiply_64_not_overflow(context: simplex::TestContext) -> anyhow::Result<()> {
    let function_index = FunctionToTest::SafeMultiply64;
    let if_test_overflow = false;
...
fn u64_test_safe_divide_64_not_overflow(context: simplex::TestContext) -> anyhow::Result<()> {
    let function_index = FunctionToTest::SafeDivide64;
    let if_test_overflow = false;
